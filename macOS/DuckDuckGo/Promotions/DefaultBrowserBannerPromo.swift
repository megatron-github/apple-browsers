//
//  DefaultBrowserBannerPromo.swift
//
...
@MainActor
final class DefaultBrowserBannerPromo: Promo {

    let id = "default-browser-banner"
    ...
    var isEligible: Bool {
        isPromoServiceEnabled() && coordinator.bannerEligibility.value
    }

    var isEligiblePublisher: AnyPublisher<Bool, Never> {
        coordinator.bannerEligibility
    ...
    func show(history: PromoHistoryRecord) async -> PromoResult {
        guard coordinator.promptTypeForEligibilityCheck() == .active(.banner) else {
            return .none
        }
