//
//  DefaultBrowserInactiveModalPromo.swift
//
...
@MainActor
final class DefaultBrowserInactiveModalPromo: Promo {

    let id = "default-browser-inactive-modal"
    let triggers: Set<PromoTrigger> = [.windowBecameKey]
    let initiated: PromoInitiated = .app
    let promoType = PromoType(severity: .high)
    let context: PromoContext = .global

    var isEligible: Bool {
        isPromoServiceEnabled() && coordinator.inactiveModalEligibility.value
    }

    var isEligiblePublisher: AnyPublisher<Bool, Never> {
        coordinator.inactiveModalEligibility
